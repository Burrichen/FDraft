 ...
 * call site can never bypass Challenge-slot locking. `isEventDraft` is
 * always `false` today (see `canEditDraftSlot`'s TODO) — there's no event
 * system on this branch yet.
 *
 ...
 * fires once every item is resolved. This is only reachable while
 * `draft.status === "active"`, i.e. before the draft has resolved at all.
 */
export async function replaceDraftSlot(
  ...
  if (
    !canEditDraftSlot({
      ...
      // TODO(event system): this branch has no `sourceEventId` field on
      // `DraftRecord` yet — wire this to `draft.sourceEventId !== null`
      // once the event system ships (see `canEditDraftSlot`'s doc comment).
      isEventDraft: false,
