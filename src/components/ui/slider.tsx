...
function Slider({
  ...
  // A single-thumb slider passes `value`/`defaultValue` as a plain number,
  // not an array — `Array.isArray` alone would fall through to the
  // range-slider default of `[min, max]` and render two thumbs for it.
      ...
      <SliderPrimitive.Control className="relative flex w-full touch-none items-center select-none data-disabled:opacity-50 data-vertical:h-full data-vertical:min-h-40 data-vertical:w-auto data-vertical:flex-col">
        ...
        {Array.from({ length: _values.length }, (_, index) => (
          <SliderPrimitive.Thumb
            ...
            key={index}
            // Root only forwards aria-labelledby to each thumb, not
            // aria-label (see @base-ui/react's SliderRoot) — passed
            // explicitly here so a single-thumb Slider's aria-label prop
            // actually reaches the focusable, keyboard-operable element.
            aria-label={ariaLabel}
            aria-labelledby={ariaLabelledBy}
