 ...
 * awarded that same way by Frontier/Signal. `"misery"` (January),
 * `"haunted"` (Halloween), and `"festive"` (Christmas) are different: each
 * of those events declares an `EventDefinition.currency` (see
 * `event-definition.ts`, docs/updates, "EVENT SYSTEM — UNIVERSAL EVENT
 * CURRENCY EARNING"), which earns their currency PER FILM WATCHED in that
 * event's own Draft, via `awardEventDraftItemReward` — not once at
 * completion.
 */
